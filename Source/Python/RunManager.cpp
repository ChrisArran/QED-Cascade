...
void RunManager::useFiniteBeaming(bool use_beaming)
{
    m_beaming = use_beaming;
}
...
void RunManager::beamOn(int events, int threads)
{
    ...
    if (m_beaming)
    {
        if (m_physics == "Classical" || m_physics == "classical")
        {
            ...
            ClassicalBeamingEmission* emission = new ClassicalBeamingEmission(m_field,
                m_timeStep, m_sampleFrac, 1.0, false);
        ...
        } else if (m_physics == "Semiclassical" || m_physics == "semiclassical")
        {
            ...
            SemiclassicalBeamingEmission* emission = new SemiclassicalBeamingEmission(m_field,
                m_timeStep, m_sampleFrac,  1.0, false);
            m_processList.push_back(emission);
        } else
        {
            m_pusher = new LorentzPusher(m_field, m_timeStep);
            StochasticBeamingEmission* emission = new StochasticBeamingEmission(m_field,
                m_timeStep, m_sampleFrac,  1.0, false);
            m_processList.push_back(emission);
        }
    }
    else
    {
        if (m_physics == "Classical" || m_physics == "classical")
        {
            ...
            ContinuousEmission* emission = new ContinuousEmission(m_field,
                m_timeStep, true, m_sampleFrac, 1.0, false);
            m_processList.push_back(emission);
        } else if (m_physics == "Semiclassical" || m_physics == "semiclassical")
        {
            m_pusher = new ModifiedLandauPusher(m_field, m_timeStep);
            ContinuousEmission* emission = new ContinuousEmission(m_field,
                m_timeStep, false, m_sampleFrac, 1.0, false);
            m_processList.push_back(emission);
        } else
        {
            m_pusher = new LorentzPusher(m_field, m_timeStep);
    ...
    {
        if (m_physics == "Classical" || m_physics == "classical" || 
            m_physics == "Semiclassical" || m_physics == "semiclassical")
        {
            ContinuousNonLinearBreitWheeler* breitWheeler = new ContinuousNonLinearBreitWheeler(m_field, 
