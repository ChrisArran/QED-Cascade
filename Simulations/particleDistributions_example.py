import time
t0 = time.time()
from QEDcascPy_positrons_example import sim_particledist_angle
import numpy as np
...
# Electron parameters
electron_energy = 2.5e9 # energy in eV
electron_energyspread = 0.2e9 # energy spread in eV
gamma0 = electron_energy/0.511e6
gamma_spread = electron_energyspread/0.511e6

# Laser parameters
Etot = 100 # J
duration0 = 25 # FWHM of intensity (fs)
waist0 = 2 # Beam waist w0 (microns)

angle = 15 # degrees
l0 = 0.1 # Stand-off distance (meters)
f0 = 0 # Focal shift (meters)

filename = '_%1.1fPW_%1.1fGeV_%1.1fmm' % (Etot/duration0,gamma0*0.511e6/1e9,l0)
params = [l0,f0]; # [length,focus shift]

# Random jitter (standard deviation)
t_std = 25e-15 # in seconds
r_std = 20e-6  # in meters
theta_std = 1e-3 # in radians
...
for j in range(Nsims):
	hist_e,bins_e1,hist_g,bins_g1,hist_p,bins_p1 = sim_particledist_angle(a0, gamma0, gamma0*0.1,angle=angle, 
					...
					Nsamples=Nsamples, up_scale=up_scale, reject = 2*waist0*1e-6, model="Quantum")
...
mean_positronrate = np.sum(positronrate)/Nsims
std_positronrate = np.sqrt(np.sum(positronrate**2)/Nsims - mean_positronrate**2)

if (Nsims<100):
	t2 = time.time()
	print("%i simulations complete after %0.2fs: " % (Nsims,t2-t1) )

exp = np.floor(np.log10(mean_positronrate))
print('length: %1.3e mm; Positron rate: (%1.3e+-%1.3f)*10^%1.3f' % (l0*1e3,mean_positronrate/10**exp,std_positronrate/10**exp,exp))
...
# Plot the rate of photon and positron production and their spectra

fig,axs = plt.subplots(2,2)

# Cumulative number of shots producing greater than a certain number of pairs
min_r = np.floor(np.log10(np.min(positronrate)*10e-12/1.6e-19))
if min_r < -4:
	min_r = -4
max_r = np.ceil(np.log10(np.max(positronrate)*10e-12/1.6e-19))
if max_r < 0:
	max_r = 0
...
axs[0,0].set_xlabel('Pairs / 10 pC > $N_+/N_-$')
...
plt.tight_layout()
plt.savefig('particleDistributions' + filename + '.png')
#plt.show()
